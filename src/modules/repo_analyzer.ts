...
export async function analyzeRepo(env: Env, opts: AnalyzeOpts): Promise<StructuredAnalysis> {
  ...
  const modelSelection = pickModelForTask({
    task: 'repo_summarize',
    hasDesignDocsOrImages: documentsToConvert.length > 0
  });
  ...
  console.log(`[AI] Starting analysis with models: primary=${modelSelection.primary}, fallback=${modelSelection.fallback}`);

  const { result: raw, modelUsed } = await callModelWithFallback(
    env,
  ...
  console.log(`[AI] Analysis completed using model: ${modelUsed}`);
  ...
  await saveStructuredAnalysis(env, repoFullName, analysis, textSamples.length, 0);
 ...
 * @deprecated Use analyzeRepo instead
 */
export async function analyzeRepoCode(env: Env, opts: AnalyzeOpts) {
  console.warn('[Deprecated] analyzeRepoCode is deprecated. Use analyzeRepo instead.');
  // For backward compatibility, delegate to the new function
  return analyzeRepo(env, opts);
}

/**
 * @description Performs structured analysis of a repository.
 * This is an alias for analyzeRepo for backward compatibility with index.ts
 */
export async function analyzeRepoCodeStructured(env: Env, opts: AnalyzeOpts) {
  return analyzeRepo(env, opts);
}
...
function pickImportantFiles(files: any[]) {
  ...
  return files
    .filter(f => !/(\.min\.(js|css)|\.(lock|map|wasm|mp4|zip|gz|tar|7z))$/i.test(f.path))
...
async function enforceEnglish(env: Env, analysis: StructuredAnalysis): Promise<StructuredAnalysis> {
  ...
  console.log(`[Translate] Non-English text detected. Attempting translation.`);
...
export type { TaskKind, PickInputs, PickResult, Env };
